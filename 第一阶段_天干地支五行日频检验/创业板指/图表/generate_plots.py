...
# ============== 配置 ==============
# 输出目录
OUTPUT_DIR = '/home/cpy/文档/金融数据库建立/关于五行的研究/创业板指/图表/'
...
def load_data():
    """加载日线、周线、月线数据和统计表"""
    df_daily = pd.read_csv('/home/cpy/文档/金融数据库建立/关于五行的研究/创业板指/数据/data_daily.csv')
    df_weekly = pd.read_csv('/home/cpy/文档/金融数据库建立/关于五行的研究/创业板指/数据/data_weekly.csv')
    df_monthly = pd.read_csv('/home/cpy/文档/金融数据库建立/关于五行的研究/创业板指/数据/data_monthly.csv')
    df_tests = pd.read_csv('/home/cpy/文档/金融数据库建立/关于五行的研究/创业板指/统计表/table_all_tests.csv')
...
def plot_daily_boxplot(df_daily, df_tests):
    ...
    # 设置标题和标签
    ax.set_title('创业板指日收益率分布 - 按天干五行分组\n(2010-2025)', 
...
def plot_weekly_rolling(df_weekly):
    ...
    # 标题和标签
    ax.set_title(f'创业板指周收益率12周滚动平均 - 按天干五行分组\n(2010-2025, n={len(df_weekly)}周)', 
                 fontsize=14, color=TEXT_COLOR, fontweight='bold', pad=20)
...
def plot_monthly_bar(df_monthly):
    ...
    # 标题和标签
    ax.set_title('创业板指月收益率时间序列 - 按天干五行着色\n(2010-2025)', 
                 fontsize=14, color=TEXT_COLOR, fontweight='bold', pad=20)
...
def plot_pvalue_summary(df_tests):
    ...
    # 标题
    ax.set_title('创业板指五行研究 - Kruskal-Wallis检验p值汇总\n(12项假设检验)', 
                 fontsize=14, color=TEXT_COLOR, fontweight='bold', pad=20)
...
def main():
    print("=" * 60)
    print("创业板指(399006.SZ) 五行研究可视化生成")
    print("=" * 60)
