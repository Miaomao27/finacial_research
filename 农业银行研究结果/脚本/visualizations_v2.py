#!/usr/bin/env python3
"""
可视化：农业银行 × 天干地支五行 - 中文修复版
生成4张核心图表（中文正常显示）+ 论文用.docx
"""
import pandas as pd
import numpy as np
import matplotlib
...
# ===== 中文修复：指定系统已有字体 =====
ZH_FONT_PATH = '/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf'
zh_font = FontProperties(fname=ZH_FONT_PATH)
zh_font_bold = FontProperties(fname=ZH_FONT_PATH, weight='bold')
zh_font_title = FontProperties(fname=ZH_FONT_PATH, weight='bold', size=15)
zh_font_subtitle = FontProperties(fname=ZH_FONT_PATH, weight='bold', size=13)
zh_font_label = FontProperties(fname=ZH_FONT_PATH, size=11)
zh_font_small = FontProperties(fname=ZH_FONT_PATH, size=9)
zh_font_tiny = FontProperties(fname=ZH_FONT_PATH, size=8)

# Set matplotlib global font
plt.rcParams.update({
...
ax.set_title('日收益率 × 天干五行', fontproperties=zh_font_subtitle)
ax.set_xlabel('天干五行', fontproperties=zh_font_label)
ax.set_ylabel('日收益率 (%)', fontproperties=zh_font_label)
for label in ax.get_xticklabels():
    label.set_fontproperties(zh_font_label)
for label in ax.get_yticklabels():
    label.set_fontproperties(zh_font_small)
...
for patch, label in zip(bp['boxes'], wx_order):
    ...
    patch.set_alpha(0.6)
ax.axhline(y=0, color='gray', linestyle='--', alpha=0.5)
ax.set_title('日收益率 × 地支五行', fontproperties=zh_font_subtitle)
ax.set_xlabel('地支五行', fontproperties=zh_font_label)
ax.set_ylabel('日收益率 (%)', fontproperties=zh_font_label)
for label in ax.get_xticklabels():
    label.set_fontproperties(zh_font_label)
for label in ax.get_yticklabels():
...
fig.suptitle('农业银行(601288) 2010-2026 日收益率按五行分组分布', fontproperties=zh_font_title)
plt.tight_layout()
...
for ax, pair_name in zip(axes.flatten(), pairs_to_plot):
    ...
    ax.set_title(f'{pair_name} 周均收益率差 (3年滚动)', fontproperties=zh_font_subtitle)
    ax.set_xlabel('窗口中心日期', fontproperties=zh_font_label)
    ax.set_ylabel('收益率差 (bps)', fontproperties=zh_font_label)
    ax.legend(prop=zh_font_small)
    for label in ax.get_xticklabels():
        label.set_fontproperties(zh_font_tiny)
    for label in ax.get_yticklabels():
        label.set_fontproperties(zh_font_small)

fig.suptitle('农业银行 五行分组周收益率 3年滚动窗口差异 (156周)', fontproperties=zh_font_title)
plt.tight_layout()
...
ax.set_title('月收益率 × 天干五行: 训练集 vs 测试集', fontproperties=zh_font_subtitle)
ax.set_ylabel('月均收益率 (%)', fontproperties=zh_font_label)
ax.legend(prop=zh_font_label)
for label in ax.get_xticklabels():
    label.set_fontproperties(zh_font_label)
for label in ax.get_yticklabels():
    label.set_fontproperties(zh_font_small)
...
for w in wx_order:
    if train_means.get(w, 0) * test_means.get(w, 0) < 0:
        ax.annotate('反转', (wx_order.index(w), 0), textcoords="offset points",
                    xytext=(0, -22), ha='center', fontsize=9, color='red', fontweight='bold',
...
ax.set_xticks(x)
ax.set_xticklabels(wx_order)
ax.set_title('月收益率 × 地支五行: 训练集 vs 测试集', fontproperties=zh_font_subtitle)
ax.set_ylabel('月均收益率 (%)', fontproperties=zh_font_label)
ax.legend(prop=zh_font_label)
for label in ax.get_xticklabels():
    label.set_fontproperties(zh_font_label)
...
fig.suptitle('农业银行 月频样本外检验 — 训练集(70%) vs 测试集(30%)', fontproperties=zh_font_title)
plt.tight_layout()
...
ax.set_xlabel('日收益率 (%)', fontproperties=zh_font_label)
ax.set_ylabel('密度', fontproperties=zh_font_label)
ax.set_title('日收益率分布 × 天干五行', fontproperties=zh_font_subtitle)
ax.legend(prop=zh_font_small)
...
ax.axhline(y=0, color='gray', linestyle='--', alpha=0.5)
ax.set_title('周均收益率 × 天干五行', fontproperties=zh_font_subtitle)
ax.set_ylabel('周均收益率 (%)', fontproperties=zh_font_label)
for label in ax.get_xticklabels():
    label.set_fontproperties(zh_font_label)
for label in ax.get_yticklabels():
    label.set_fontproperties(zh_font_small)
...
ax.set_title('农业银行月累计收益 (2010-2026)', fontproperties=zh_font_subtitle)
ax.set_ylabel('累计净值', fontproperties=zh_font_label)
ax.legend(prop=zh_font_small)
...
ax.set_yticklabels(test_names, fontproperties=zh_font_small)
ax.axvline(x=0.05, color='red', linestyle='--', alpha=0.7, label='α=0.05')
ax.set_xlabel('p值', fontproperties=zh_font_label)
ax.set_title('所有假设检验p值汇总', fontproperties=zh_font_subtitle)
ax.legend(prop=zh_font_small)
for label in ax.get_xticklabels():
    label.set_fontproperties(zh_font_small)
for i, p in enumerate(p_values):
    ax.text(p + 0.01, i, f'{p:.3f}', va='center', fontproperties=zh_font_small)
