#!/usr/bin/env python3
"""
五粮液(000858.SZ)股价与天干地支五行关系检验 - 完整分析管道
...
for wk, grp in df.groupby('week_number', sort=True):
    ...
    weekly_data.append({
        ...
        'week_tg_wuxing': first_day['day_tg_wuxing'],
        'week_dz_wuxing': first_day['day_dz_wuxing'],
...
# 保存中间数据
df.to_csv(f'{OUTPUT_DIR}/数据/data_daily_with_ganzhi.csv', index=False, encoding='utf-8-sig')
df_weekly.to_csv(f'{OUTPUT_DIR}/数据/data_weekly.csv', index=False, encoding='utf-8-sig')
df_monthly.to_csv(f'{OUTPUT_DIR}/数据/data_monthly.csv', index=False, encoding='utf-8-sig')
print(f"\n  数据已保存至: {OUTPUT_DIR}/数据/")
...
# 描述统计表保存
tg_groups.round(6).to_csv(f'{OUTPUT_DIR}/统计表/table_daily_tg_wuxing.csv', encoding='utf-8-sig')
dz_groups.round(6).to_csv(f'{OUTPUT_DIR}/统计表/table_daily_dz_wuxing.csv', encoding='utf-8-sig')
...
df_roll.to_csv(f'{OUTPUT_DIR}/统计表/table_weekly_rolling.csv', index=False, encoding='utf-8-sig')
...
df_results.to_csv(f'{OUTPUT_DIR}/统计表/table_all_tests.csv', index=False, encoding='utf-8-sig')
print(f"\n  检验结果表已保存")
...
# 生成结论报告 markdown
conclusion = f"""# {STOCK_NAME}({STOCK_CODE})股价与天干地支五行关系：统计检验结论报告

**日期**: {datetime.now().strftime('%Y年%m月%d日')}  
**数据范围**: {df['trade_date'].min().strftime('%Y-%m-%d')} ~ {df['trade_date'].max().strftime('%Y-%m-%d')} ({len(df)}个交易日)  
...
if n_sig == 0:
    conclusion += f"""**天干地支五行对{STOCK_NAME}股价没有统计上显著的预测能力。** {len(df_results)}个假设检验无一显著，结果与农业银行(601288)研究保持一致。
"""
else:
    conclusion += f"""**在{len(df_results)}个检验中发现{n_sig}个显著结果，需谨慎解读。**
"""
...
for wx in ['金', '木', '水', '火', '土']:
    if wx in tg_groups.index:
        val = tg_groups.loc[wx]
        conclusion += f"| {wx} | {val['均值%']:+.4f}% | {val['标准差%']:.4f}% | {val['上涨占比']:.1%} |\n"

conclusion += f"""
**Kruskal-Wallis检验: p={p_tg:.6f}**

"""

if p_tg > 0.05:
    conclusion += "5组收益率无统计上显著差异。\n"
else:
    conclusion += "注意：日频差异虽统计显著，但效应量极小，需周/月频进一步验证。\n"
...
for _, row in df_roll.iterrows():
    conclusion += f"| {row['pair']} | {row['mean_diff_bps']:+.2f} | {row['t_stat']:+.3f} |\n"

conclusion += f"""
Kruskal-Wallis: 周天干 p={pw_tg:.6f}, 周地支 p={pw_dz:.6f}
...
| 检验 | H统计量 | p值 | 显著? |
| ---- | ------- | --- | ----- |
| 月天干(训练集) | {hm_tg:.4f} | {pm_tg:.6f} | {'✅' if pm_tg<0.05 else '❌'} |
| 月地支(训练集) | {hm_dz:.4f} | {pm_dz:.6f} | {'✅' if pm_dz<0.05 else '❌'} |
| 月天干(测试集) | {hm_tg_test:.4f} | {pm_tg_test:.6f} | {'✅' if pm_tg_test<0.05 else '❌'} |
| 月地支(测试集) | {hm_dz_test:.4f} | {pm_dz_test:.6f} | {'✅' if pm_dz_test<0.05 else '❌'} |

**训练集最强规律**: {hypothesis}

**样本外验证方向一致**: {'✅' if direction_ok else '❌'}
"""
...
| 模型 | R² | F检验p | 五行变量显著? |
| ---- | -- | ------ | ------------ |
| 仅五行 | {model1.rsquared:.4f} | {model1.f_pvalue:.6f} | {'✅' if model1.f_pvalue < 0.05 else '❌'} |
| +阳历月份 | {model2.rsquared:.4f} | {model2.f_pvalue:.6f} | {'✅' if any_tg_sig else '❌'} |
| +年份 | {model3.rsquared:.4f} | {model3.f_pvalue:.6f} | - |
"""
...
| 维度 | 农业银行(601288) | {STOCK_NAME}({STOCK_CODE}) |
| ---- | --------------- | ------------------------ |
| 数据量 | 3,833个交易日 | {len(df)}个交易日 |
| 显著检验数 | 0/9 | {n_sig}/{len(df_results)} |
| 日频p值 | 0.917 | {p_tg:.4f} |
| 月频测试p值 | 0.141~0.272 | {pm_tg_test:.4f}~{pm_dz_test:.4f} |
| 方向一致 | ❌ | {'✅' if direction_ok else '❌'} |
...
for _, row in df_results.iterrows():
    conclusion += f"| {row['阶段']} | {row['H统计量']:.4f} | {row['p值']:.6f} | {'✅' if row['显著(α=0.05)'] else '❌'} |\n"

with open(f'{OUTPUT_DIR}/结论报告.md', 'w', encoding='utf-8') as f:
    f.write(conclusion)
print(f"  结论报告已保存: {OUTPUT_DIR}/结论报告.md")
